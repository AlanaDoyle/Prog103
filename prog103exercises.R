...
# “senescent”.
age_class <- c("egg", "larva", "adult", "senescent")
find_y <- function(age_class){
  if (age_class == "egg") {
    print(y <- 10)
  } else if (age_class == "larva") {
    print(y <- 5)
...
shortfin_mako_mass_kg <- function(fork_length){
  result <- a * fork_length^b
  return(result)
}
